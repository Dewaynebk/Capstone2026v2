"""Classify IoT traffic records using the trained IoT-23 ML model."""
    ...
    def prepare_input(path: Path) -> pd.DataFrame:
        ...
        if path.suffix.lower() == ".csv":
            ...
            # If user passes another Zeek log, preprocess it into the standard file then load.
    ...
    def write_dashboard(df: pd.DataFrame, dashboard_path: Path):
        ...
        cols = [c for c in ["source_ip", "destination_ip", "protocol", "service", "destination_port",
                            "duration", "orig_bytes", "resp_bytes", "orig_pkts", "resp_pkts",
                            "prediction", "confidence"] if c in df.columns]

        all_table = df[cols].head(200).to_html(index=False)
        alert_table = alerts[cols].to_html(index=False) if len(alerts) else "<p>No malicious traffic detected.</p>"
...
{all_table}
    ...
    def main():
        ...
        parser.add_argument("--input", default=str(PROCESSED_DATA), help="Processed CSV or Zeek conn.log.labeled file")
