...
- It does not require the full raw IoT-23 conn.log.labeled file when a trained model already exists.
- It can classify the sample CSV included in the GitHub repository.
"""
...
from config import FEATURE_COLUMNS, OUTPUT_DIR, SELECTED_MODEL, PROCESSED_DATA, ROOT
from train_models import train_models


def ensure_model_exists():
    """Ensure a trained model exists before prediction."""
    if SELECTED_MODEL.exists():
        return

    if PROCESSED_DATA.exists():
        print("No saved model found. Training from processed dataset first...")
        train_models()
        return

    raise FileNotFoundError(
        "No trained model found at models/selected_model.joblib and no processed dataset found.\n"
        "For the GitHub demo, make sure the models folder includes selected_model.joblib.\n"
        "If using the full dataset, place conn.log.labeled in data/raw and run preprocessing/training first."
    )


def prepare_input(path: Path) -> pd.DataFrame:
    """Load a processed/sample CSV file and make sure model features exist."""
    if path.suffix.lower() != ".csv":
        raise ValueError(
            "Demo mode expects a CSV input file. Use data/sample/sample_iot23_dataset.csv."
        )
...
def main():
    ...
    parser.add_argument(
        "--input",
        default=str(ROOT / "data" / "sample" / "sample_iot23_dataset.csv"),
        help="Path to input sample CSV file"
    )
